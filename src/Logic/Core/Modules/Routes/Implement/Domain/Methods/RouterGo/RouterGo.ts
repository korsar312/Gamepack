...
import { generatePath, NavigateFunction, Params } from "react-router-native";
...
export class RouterGo {
	static execute() {
		...
		return function (
			...
			function link() {
				navigate && navigate(generatePath(String(RouteInterfaces.PathName[path]), params), { replace: false, state });
			}
		};
	}
}
