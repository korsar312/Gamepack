...
export class GetText {
	static execute() {
		return function (
			...
			const isVariables = !!theme;

			const findWord = notTranslate
				? String(wordKey)
				: languageDictionary?.[wordKey as LanguageInterface.TAllTranslateWord]?.[langModel] || String(wordKey);

			const transArr = isVariables && addon?.trans?.[wordKey as LanguageInterface.EWord]?.[langModel]?.[theme];
			const modifyArr = isVariables && addon?.modify?.[wordKey as LanguageInterface.EWord]?.[langModel]?.[theme];
...
function createJSX(children?: ReactNode, css?: CSSProperties) {
